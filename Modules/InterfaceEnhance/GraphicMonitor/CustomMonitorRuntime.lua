...
-- 环形纹理路径格式
local RING_TEXTURE_FMT = "Interface\\AddOns\\YuXuanSpecial\\Assets\\GraphicMonitor\\Ring_%spx.tga"
...
local function segmentLayoutSignature(cfg, barFrame)
    ...
    return table.concat({
        ...
        tostring(tonumber(cfg.stackThreshold3) or 0),
        colKey(cfg.stackColor1),
        colKey(cfg.stackColor2),
        colKey(cfg.stackColor3),
        colKey(cfg.barColor),
        colKey(cfg.bgColor),
...
local function CreateSegments(barFrame, count, cfg, isStack, isRing)
    ...
    local t3         = isStack and (tonumber(cfg.stackThreshold3) or 0) or 0
    local c1         = cfg.stackColor1 or { r = 1, g = 0.5, b = 0, a = 1 }
    local c2         = cfg.stackColor2 or { r = 1, g = 0, b = 0, a = 1 }
    local c3         = cfg.stackColor3 or { r = 0.65, g = 0.2, b = 1, a = 1 }
    ...
    for i = 1, count do
        ...
        if isStack then
            ...
            if t3 > 0 then
                local ov3 = CreateFrame("StatusBar", nil, segFrame)
                ov3:SetAllPoints(segFrame)
                ov3:SetStatusBarTexture(tex)
                ov3:SetStatusBarColor(c3.r, c3.g, c3.b, c3.a)
                ov3:SetValue(0)
                ov3:EnableMouse(false)
                ov3:SetFrameLevel(baseLevel + 4)
                ov3:SetMinMaxValues((i < t3) and (t3 - 1) or (i - 1), (i < t3) and t3 or i)
                if BFK then
                    BFK.ConfigureStatusBar(ov3)
                    BFK.SetOrientation(ov3, dir)
                    BFK.SetReverseFill(ov3, barReverse)
                end
                table.insert(barFrame._thresholdOverlays, ov3)
            end
        end

        local borderFrame = CreateFrame("Frame", nil, segFrame)
        borderFrame:SetFrameLevel(baseLevel + 5)
