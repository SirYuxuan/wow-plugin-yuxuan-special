...
local function destroyContainer(storeKey, spellID)
    ...
    if NS.GraphicMonitorRuntime then
        NS.GraphicMonitorRuntime.onContainerDestroyed(storeKey, spellID)
    end
...
local function ensureContainer(storeKey, spellID, cfg)
    ...
    if NS.GraphicMonitorRuntime then
        NS.GraphicMonitorRuntime.onContainerReady(storeKey, spellID, cfg, container)
    end
        ...
        if cfg.enabled and hasVisualOutput(cfg) then
            if checkIsValid(storeKey, spellID) then
                ...
                if cont then
                    ...
                    elseif NS.GraphicMonitorRuntime and NS.GraphicMonitorRuntime.syncBarConfig then
                        NS.GraphicMonitorRuntime.syncBarConfig(storeKey, spellID, cfg)
                    end
                else
                    ensureContainer(storeKey, spellID, cfg)
                end
...
local function refreshContainerGeometry(storeKey, spellID, cfg)
    ...
    if (not cw or math.abs(cw - w) > 0.5) or (not ch or math.abs(ch - h) > 0.5) then
        PP.SetSize(container, w, h)
        if NS.GraphicMonitorRuntime and NS.GraphicMonitorRuntime.notifyContainerGeometryChanged then
            NS.GraphicMonitorRuntime.notifyContainerGeometryChanged(storeKey, spellID)
        end
    end
    VFlow.ContainerAnchor.ApplyFramePosition(container, cfg, nil)
    if VFlow.DragFrame and VFlow.DragFrame.applyRegisteredPosition then
